    ...
    <el-dialog v-model="groupDialogVisible" :title="editingGroup ? '编辑分组' : '新增分组'" width="420px">
      <el-form :model="groupForm" label-position="top">
        <el-form-item label="分组名称">
          <el-input v-model="groupForm.name" placeholder="如：2024年获奖" maxlength="50" show-word-limit />
        </el-form-item>
...
import {
  getAllHonorData, createHonorGroup, updateHonorGroup, deleteHonorGroup, sortHonorGroups,
  addPhoto, updatePhoto, deletePhoto, sortPhotos, uploadImage
} from '../../api'
...
async function loadGroups() {
  try {
    const data = await getAllHonorData()
...
async function handleSaveGroup() {
  ...
  try {
    if (editingGroup.value) {
      await updateHonorGroup(editingGroup.value.id, groupForm.value)
      ElMessage.success('分组已更新')
    } else {
      await createHonorGroup(groupForm.value)
...
async function handleGroupSort() {
  try {
    await sortHonorGroups(groups.value.map(g => ({ id: g.id })))
...
async function handlePhotoUpload({ file }) {
  ...
  try {
    ...
    await addPhoto(activeGroupId.value, { image_url: res.url, title: '' })
...
async function handleEditPhoto(photo) {
  try {
    await updatePhoto(photo.id, { title: photo.title })
  } catch (e) {
    // 错误已在拦截器处理
  }
}
...
async function handlePhotoSort() {
  try {
    await sortPhotos(activeGroupId.value, activePhotos.value.map(p => ({ id: p.id })))
    ElMessage.success('排序已更新')
  } catch (e) {
    // 错误已在拦截器处理
  }
