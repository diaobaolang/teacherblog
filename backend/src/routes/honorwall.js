...
router.get('/groups', async (req, res) => {
  const { data } = await pgClient.select('photo_groups', {
    columns: '*',
...
router.get('/groups/:id/photos', async (req, res) => {
  const { data } = await pgClient.select('photos', {
    columns: '*',
...
router.get('/all', async (req, res) => {
  const { data: groups } = await pgClient.select('photo_groups', {
    columns: '*',
    order: 'sort_order.asc,id.asc',
  });
  const { data: allPhotos } = await pgClient.select('photos', {
    columns: '*',
    order: 'sort_order.asc,id.asc',
  });
...
router.post('/groups', authMiddleware, async (req, res) => {
  ...
  const maxSort = await pgClient.max('photo_groups', 'sort_order');
  const sortOrder = (maxSort || 0) + 1;
  const row = await pgClient.insert('photo_groups', {
...
router.put('/groups/:id(\\d+)', authMiddleware, async (req, res) => {
  ...
  await pgClient.update('photo_groups', {
...
router.delete('/groups/:id', authMiddleware, async (req, res) => {
  ...
  await pgClient.delete('photos', { group_id: id });
  await pgClient.delete('photo_groups', { id });
...
router.put('/groups/sort', authMiddleware, async (req, res) => {
  ...
  for (let i = 0; i < items.length; i++) {
    await pgClient.update('photo_groups', { sort_order: i + 1 }, { id: items[i].id });
  }
...
router.post('/groups/:id/photos', authMiddleware, async (req, res) => {
  ...
  const group = await pgClient.getOne('photo_groups', { filters: { id: groupId } });
  if (!group) {
    return res.status(404).json({ error: '分组不存在' });
  }
  const maxSort = await pgClient.max('photos', 'sort_order', { group_id: groupId });
  const sortOrder = (maxSort || 0) + 1;
  const row = await pgClient.insert('photos', {
...
router.put('/photos/:id', authMiddleware, async (req, res) => {
  const { id } = req.params;
  const { image_url, title } = req.body;
  const existing = await pgClient.getOne('photos', { filters: { id } });
  if (!existing) {
    return res.status(404).json({ error: '照片不存在' });
  }
  await pgClient.update('photos', {
...
router.delete('/photos/:id', authMiddleware, async (req, res) => {
  ...
  await pgClient.delete('photos', { id });
...
router.put('/groups/:id/photos/sort', authMiddleware, async (req, res) => {
  ...
  for (let i = 0; i < items.length; i++) {
    await pgClient.update('photos', { sort_order: i + 1 }, { id: items[i].id, group_id: groupId });
  }
  res.json({ message: '排序已更新' });
});
