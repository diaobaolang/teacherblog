...
router.get('/groups', (req, res) => {
  const groups = db.prepare('SELECT * FROM photo_groups ORDER BY sort_order ASC, id ASC').all();
  res.json(groups);
});

// 获取某个分组下的所有照片
router.get('/groups/:id/photos', (req, res) => {
  const photos = db.prepare('SELECT * FROM photos WHERE group_id = ? ORDER BY sort_order ASC, id ASC').all(req.params.id);
  res.json(photos);
});

// 获取所有分组及其照片（一次性返回，首页轮播用）
router.get('/all', (req, res) => {
  const groups = db.prepare('SELECT * FROM photo_groups ORDER BY sort_order ASC, id ASC').all();
  const allPhotos = db.prepare('SELECT * FROM photos ORDER BY sort_order ASC, id ASC').all();
...
router.post('/groups', authMiddleware, (req, res) => {
  ...
  const maxOrder = db.prepare('SELECT MAX(sort_order) as max FROM photo_groups').get();
  const sortOrder = (maxOrder.max || 0) + 1;
  const result = db.prepare('INSERT INTO photo_groups (name, cover_image, sort_order) VALUES (?, ?, ?)')
...
router.put('/groups/:id', authMiddleware, (req, res) => {
  const { id } = req.params;
  const { name, cover_image } = req.body;
  const existing = db.prepare('SELECT * FROM photo_groups WHERE id = ?').get(id);
  if (!existing) {
    return res.status(404).json({ error: '分组不存在' });
  }
  db.prepare('UPDATE photo_groups SET name = ?, cover_image = ? WHERE id = ?')
...
router.delete('/groups/:id', authMiddleware, (req, res) => {
  ...
  db.prepare('DELETE FROM photos WHERE group_id = ?').run(id);
  db.prepare('DELETE FROM photo_groups WHERE id = ?').run(id);
  db.save();
...
router.put('/groups/sort', authMiddleware, (req, res) => {
  ...
  const update = db.prepare('UPDATE photo_groups SET sort_order = ? WHERE id = ?');
...
router.post('/groups/:id/photos', authMiddleware, (req, res) => {
  ...
  const group = db.prepare('SELECT * FROM photo_groups WHERE id = ?').get(groupId);
  if (!group) {
    return res.status(404).json({ error: '分组不存在' });
  }
  const maxOrder = db.prepare('SELECT MAX(sort_order) as max FROM photos WHERE group_id = ?').get(groupId);
  const sortOrder = (maxOrder.max || 0) + 1;
  const result = db.prepare('INSERT INTO photos (group_id, image_url, title, sort_order) VALUES (?, ?, ?, ?)')
...
router.put('/photos/:id', authMiddleware, (req, res) => {
  ...
  db.prepare('UPDATE photos SET image_url = ?, title = ? WHERE id = ?')
...
router.put('/groups/:id/photos/sort', authMiddleware, (req, res) => {
  ...
  const update = db.prepare('UPDATE photos SET sort_order = ? WHERE id = ? AND group_id = ?');
  const tx = db.transaction(() => {
    items.forEach((item, index) => {
